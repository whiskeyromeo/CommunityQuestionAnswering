...
class Word2VecSimilarity:
    ...
    def init(self, allQuestions):
        ...
        NEGATIVE = 10
        cleanQuestions(allQuestions)
        questionList = generateQuestionList(allQuestions)
        id2word = gensim.corpora.Dictionary(questionList)
        word2id = dict((v,k) for k,v in id2word.iteritems())
        corpus = lambda: ([word.lower() for word in q if word in word2id] for q['question_words'] in allQuestions)
        model = gensim.models.Word2Vec(size=DIM, window=WINDOW, workers=WORKERS,hs=0,negative=NEGATIVE)
        model.build_vocab(corpus())
        model.train(corpus())
        for q in allQuestions:
            allQuestions[q]['word2vec'] = generateQuestionVector(model, questionDict[q]['clean_question_words'], DIM)
            for r in allQuestions[q]['related']:
                allQuestions[q]['related'][r]['word2vec'] = generateQuestionVector(model, questionDict[q]['clean_question_words'], DIM)
        return
