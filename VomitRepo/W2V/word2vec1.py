...
for i,q in enumerate(questionList):
	split = nltk.word_tokenize(q)
	split = [w for w in split if w not in stops]
	questionTokens.append(split)
	questions[i]['question_tokens'] = split

model = Word2Vec(questionTokens, size=100, window=5,min_count=5, workers=4)
...
def generateQuestionVector(questions, model):
	#Need to generate a model vector for each sentence based off of the average of the 
	# vectors included in the sentence.
	# this will require summation of the vectors into a new space. 
	for i,q in enumerate(questions):



def BuildWord2VecMap(hashmap, comments=False):
